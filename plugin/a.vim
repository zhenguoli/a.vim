...
" Function : AlternateFileForce (PUBLIC)
" Purpose  : Opens a new buffer. When g:alternateNoDefaultAlternate is set,
"            creating the buffer.
" Args     : accepts one argument. If present it used the argument as the new
"            extension.
" Returns  : nothing
" Author   : zhenguoli
function! AlternateFileForce(splitWindow, ...)
    let saveDefaultAlternate = g:alternateNoDefaultAlternate
    let g:alternateNoDefaultAlternate = 0
    call AlternateFile(a:splitWindow)
    let g:alternateNoDefaultAlternate = saveDefaultAlternate
endfunction
...
comm! -nargs=? -bang AF call AlternateFileForce("n<bang>", <f-args>)
